...
func TestWireDecodersRejectMalformedJSONStringEncoding(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := tt.decode(tt.body); err == nil {
				t.Fatalf("decoder accepted malformed JSON string encoding: %q", tt.body)
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := tt.validate(); err == nil {
				t.Fatal("Validate accepted a malformed raw JSON string")
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := tt.decode([]byte(tt.body)); err == nil {
				t.Fatalf("standalone wire record accepted malformed JSON string encoding: %q", tt.body)
			}
		})
	}
}
